...
bool CommandBus::_push(const Command& cmd) {
    ...
    if (next == _tail) { _recordDrop(cmd); return false; }  // ring full
    _ring[_head] = cmd;
    __dmb();
    _head = next;
    return true;
...
// ─────────────────────────────────────────────────────────────
//  _recordDrop() — records one dropped command into the diagnostic
//  log (see the public accessors' comment in command_bus.h). Called
//  only from _push(), which per dispatch()'s own core check only ever
//  runs on core1 — so this is single-core access, same core as
//  Webserver's _panelSysteem() reads it from, needing no memory
//  barrier or locking.
// ─────────────────────────────────────────────────────────────
void CommandBus::_recordDrop(const Command& cmd) {
    _dropTotal++;
    _dropLog[_dropLogHead] = { millis(), cmd.sourceId, cmd.type };
    _dropLogHead = (_dropLogHead + 1) % CMD_BUS_DROP_LOG_SIZE;
    if (_dropLogCount < CMD_BUS_DROP_LOG_SIZE) _dropLogCount++;
}

// dropLogEntry() — see the declaration's comment in command_bus.h.
// i=0 is the most recent drop, working backwards from _dropLogHead
// (the slot the NEXT entry would be written to, so the most recent
// entry is one slot before it, wrapping around at 0 as needed).
const DispatchDropEntry& CommandBus::dropLogEntry(uint8_t i) const {
    uint8_t idx = (_dropLogHead + CMD_BUS_DROP_LOG_SIZE - 1 - i) % CMD_BUS_DROP_LOG_SIZE;
    return _dropLog[idx];
}
