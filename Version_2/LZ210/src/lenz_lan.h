...
//
//  Implements the Lenz LI-USB-Ethernet protocol (LAN v3.6). Listens
//  on the TCP port configured via EEPROM key "lenz.port" (web
//  interface, default 5550). Supports up to LENZ_LAN_MAX_CLIENTS
...
public:
    ...
    // Starts the TCP server on the configured port (EEPROM
    // "lenz.port", default 5550), registers EventBus handler
    void begin()  override;
...
private:
    ...
    // "lenz.port") is known — EthernetServer's port is fixed at
    // construction time with no way to change it afterwards, so a
    // plain value member here would permanently bake in whatever port
    // was current when the member was default-constructed, before
    // EEPROM has even loaded. A pointer, allocated once in begin(),
    // lets the actual configured port take effect. Previously this
    // was `EthernetServer _server{5550};`, silently ignoring the
    // "lenz.port" web-interface setting entirely.
