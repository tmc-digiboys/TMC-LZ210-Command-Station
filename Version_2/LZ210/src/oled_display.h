...
// Timeout for the large per-page I2C write in _writePage() (129 bytes:
// 1 control byte + 128 data bytes). Computed from the actual configured
// I2C clock (OLED_I2C_FREQ) rather than a fixed value, with a 3x safety
// margin, so it always comfortably covers the theoretical minimum
// transfer time regardless of which frequency is configured — a fixed
// 10ms value was previously hardcoded here, which is fine at 400kHz
// (~2.9ms theoretical minimum) but is SHORTER than the ~11.6ms a full
// page write actually needs at 100kHz, silently truncating the
// transfer mid-page and corrupting the display (observed as an
// alternating-line/striped pattern in each character's glyph — worse,
// not better, at the lower clock speed, which is the tell-tale sign of
// a timeout that is too short rather than a signal-integrity issue).
#define OLED_PAGE_WRITE_TIMEOUT_US \
    (((uint32_t)(OLED_WIDTH + 1) * 9 * 3 * 1000000UL) / OLED_I2C_FREQ)
