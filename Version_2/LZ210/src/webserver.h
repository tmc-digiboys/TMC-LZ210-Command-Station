...
#include "dcc_current_monitor.h"   // AckState, for _ackStateName()
#include "command_bus.h"          // CommandBus, DispatchDropEntry, for _panelSysteem()
#include <Ethernet.h>
...
public:
    ...
    // Starts the HTTP server on the configured port (EEPROM
    // "web.port", default WEB_PORT)
    void begin()  override;
...
private:
    // Constructed in begin() once the configured port (EEPROM key
    // "web.port") is known — see LenzLan's identical pattern in
    // lenz_lan.h for the full reasoning. Previously
    // `EthernetServer _server{WEB_PORT};`, silently ignoring the
    // "web.port" web-interface setting entirely.
    ...
    static const __FlashStringHelper* _ackStateName(AckState s);

    // Short display string for a CmdType value, used in
    // _panelSysteem()'s CommandBus drop log (see command_bus.h).
    static const __FlashStringHelper* _cmdTypeName(CmdType t);
    ...
    void _panelPower    (EthernetClient& c);  // Power rail monitoring (v2 hardware)
    ...
    void _panelLoconet  (EthernetClient& c);  // LocoNet slot table (LocoNet-controlled locos only)
