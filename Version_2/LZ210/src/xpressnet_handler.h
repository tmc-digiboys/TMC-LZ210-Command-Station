...
//    handler.setBroadcastCallback([](const uint8_t* d, uint8_t l, uint16_t addr) { ... });
...
// XnBroadcastFn — locoAddr is the loco this broadcast is about, or 0
// as a sentinel for broadcasts that aren't about a specific loco
// (track power, e-stop, turnout feedback, fast clock, ...). Added so
// a transport whose broadcast format differs from genuine RS485
// XpressNet's (e.g. Z21 LAN, whose LAN_X_LOCO_INFO needs the address
// explicitly, unlike RS485's own address-less loco-info reply — see
// z21_lan.cpp's setBroadcastCallback()) can rebuild a properly-
// formatted, correctly-addressed message instead of forwarding the
// RS485-native bytes unchanged.
using XnBroadcastFn = void(*)(const uint8_t* data, uint8_t len, uint16_t locoAddr);
...
struct CentraleState {
    ...
    bool     progResultReady = false;  // True if a definite CV read/write result
                                       // (success OR failure) is available to report
    bool     progResultOk    = false;  // True=success (progResultData valid),
                                       // false=no ACK/decoder found or timed out —
                                       // see DccHal::_exitServiceMode() in dcc_hal.cpp
...
public:
    ...
    // Read-only access to _srcId (set via begin()) — lets shared code
    // that holds a reference to some XpressNetHandler instance (e.g.
    // XnInterfaceLayer::process(), used by both LenzLan and LenzUsb)
    // identify which transport this particular instance serves, e.g.
    // for labelling trace-log lines correctly per instance.
    uint8_t srcId() const { return _srcId; }

    // Builds an error response with the given error code. Static and
    // stateless (only uses its parameters and the static xorCheck()),
    // so XnInterfaceLayer can also build a consistently-formatted
    // error reply for a failed XOR check on its own interface-local
    // commands, without needing an XpressNetHandler instance.
    static uint8_t buildError(uint8_t* buf, uint8_t errCode);
...
private:
    ...
    // dispatches LOCO_SPEED command to DCC HAL. Returns false if the
    // CommandBus dispatch itself failed (ring buffer full — see
    // command_bus.h); the repository update above still happened
    // regardless, since that's independent of the CommandBus.
    bool updateLoco(uint16_t addr, uint8_t speed, uint8_t dir,
