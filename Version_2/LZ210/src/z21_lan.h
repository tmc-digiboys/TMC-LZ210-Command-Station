...
//
//  Implements the Z21 LAN protocol v1.10 over UDP. Listens on the
//  port configured via EEPROM key "z21.port" (web interface, default
//  Z21_PORT/21105).
...
//    events it receives. Clients are removed after the configured
//    inactivity timeout ("z21.client_timeout_s" in EEPROM, web
//    interface, default Z21_CLIENT_TIMEOUT_DEFAULT_MS) — see
//    _checkTimeouts().
//
//  Z21 frame format:
...
// Default client inactivity timeout (ms) — matches the official Z21
// LAN Protocol Specification's own requirement: "Each client is
// expected to communicate with the Z21 once per minute, otherwise it
// will be removed from the list of active participants." Previously
// hardcoded to 30000 (30s) — half the spec's own interval, so a
// genuinely spec-compliant client (or a human manually testing, with
// natural pauses between actions) could get incorrectly dropped,
// silently losing its broadcast subscription (confirmed via packet
// capture, Rob: turnout broadcasts stopped arriving ~30-170s after
// SetBroadcastFlags, resuming only after re-sending it). Now EEPROM-
// configurable ("z21.client_timeout_s") with this as the default,
// read live in _checkTimeouts().
#define Z21_CLIENT_TIMEOUT_DEFAULT_MS 60000
...
public:
    ...
    // Starts UDP listener on the configured port (EEPROM "z21.port",
    // default Z21_PORT), registers EventBus handler
    void begin()  override;
...
private:
    ...
    // _handleCvPomRead()/_handleCvPomWrite() removed — the headers
    // they were dispatched from (0x00A4/0x00A8) collided with genuine
    // LAN_LOCONET_DETECTOR (0x00A4) and did not correspond to any
    // real, documented Z21 header. Genuine POM operations are handled
    // via the LAN_X tunnel instead (X-Header 0xE6, DB0=0x30) — see
    // _handleXBus() in the .cpp.
    ...
    // Removes clients that have exceeded the configured inactivity
    // timeout ("z21.client_timeout_s", read live from EEPROM)
    void       _checkTimeouts();
