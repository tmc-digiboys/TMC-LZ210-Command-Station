...
//  the TCP server listening on port 5550 and resets every client
...
// ─────────────────────────────────────────────────────────────
void LenzLan::begin() {
    Serial2.print("LenzLan begin: Ethernet link=");
    Serial2.print(Ethernet.linkStatus());
    Serial2.print(" IP="); Serial2.println(Ethernet.localIP());
    ...
    Serial2.println("LenzLan: server gestart op poort 5550");
    ...
    sXpressNet.setBroadcastCallback([](const uint8_t* data, uint8_t dlen) {
...
void LenzLan::_acceptNew() {
    ...
    Serial2.print("LenzLan: nieuwe client van ");
    Serial2.println(newClient.remoteIP());
...
void LenzLan::_processClient(LenzClient& c) {
    if (!c.tcp.connected() && !c.tcp.available()) {
        Serial2.println("LenzLan: client verbroken");
        c.tcp.stop();
        c.active   = false;
        c.progMode = false;
        _connCount--;
    ...
    if (c.rxLen >= LENZ_LAN_RX_BUF && c.tcp.available()) {
        Serial2.println("LenzLan: buffer-overflow, herstel...");
        uint8_t err[3];
    ...
    if (c.rxLen == 0) {
        ...
        if (c.frameStartMs == 0) c.frameStartMs = millis();
            ...
            Serial2.println("LenzLan: onvolledig frame timeout");
            uint8_t err[3];
