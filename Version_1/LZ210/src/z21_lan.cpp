...
namespace Z21Hdr {
    ...
    constexpr uint16_t LAN_CV_POM_READ       = 0x00A4;
    constexpr uint16_t LAN_CV_POM_WRITE      = 0x00A8;
...
//
//  Opens the UDP listener on Z21_PORT, clears all client slots to
//  their inactive/empty state, resets the connected-client counter,
//  and registers this module's onEvent() method as the EventBus
//  handler for ModuleId::Z21_LAN, so hardware events published by
//  other modules (power state, loco state, RS-Bus feedback) are
//  translated into Z21 broadcasts.
// ─────────────────────────────────────────────────────────────
void Z21Lan::begin() {
    _udp.begin(Z21_PORT);
...
void Z21Lan::_processPacket(const uint8_t* buf, uint16_t len,
    ...
    switch (hdr) {
        ...
        case Z21Hdr::LAN_CV_POM_READ:       _handleCvPomRead(*c, data, dataLen);    break;
        case Z21Hdr::LAN_CV_POM_WRITE:      _handleCvPomWrite(*c, data, dataLen);   break;
        default: break;
    }
}
...
void Z21Lan::_handleXBus(Z21Client& c,
    ...
    if (!sZ21Init) {
        ...
        sZ21XpressNet.setBroadcastCallback([](const uint8_t* bdata, uint8_t dlen) {
            XnFrame xf;
            if (dlen > 0 && dlen <= (int)sizeof(xf.data)) {
                ...
                // everything else (loco status etc.) → DRIVING_LOCO | SYSTEM_STATE
    ...
    if (len >= 4 && data[0] == 0x53) {
        uint16_t outAddr = ((uint16_t)data[1] << 8) | data[2];  // 1-based
        if (outAddr == 0) outAddr = 1;
        ...
        uint8_t  TT      = out0 % 4;
        // XpressNet Dat: 1 0 0 0 C TT1 TT0 R
        uint8_t dat = 0x80 | (activate << 3) | (TT << 1) | output;
        mappedData[0] = 0x53;
        mappedData[1] = (decoder >> 8) & 0xFF;
        mappedData[2] =  decoder       & 0xFF;
        mappedData[3] = dat;
        mappedData[4] = mappedData[0]^mappedData[1]^mappedData[2]^mappedData[3];
        procData = mappedData;
        procLen  = 5;
    }
...
// ─────────────────────────────────────────────────────────────
// _handleCvPomRead() — LAN_CV_POM_READ: decodes the target loco
// address and the CV number to read, and dispatches a POM_READ
// command (Programming On Main — CV access while the loco stays on
// the main track) onto the command bus.
// ─────────────────────────────────────────────────────────────
void Z21Lan::_handleCvPomRead(Z21Client& c, const uint8_t* data, uint8_t len) {
    if (len < 4) return;
    Command cmd{};
    cmd.type     = CmdType::POM_READ; cmd.sourceId = ModuleId::Z21_LAN;
    cmd.address  = _u16le(data);
    cmd.cv       = _u16le(data + 2) + 1;
    CommandBus::instance().dispatch(cmd);
}

// ─────────────────────────────────────────────────────────────
// _handleCvPomWrite() — LAN_CV_POM_WRITE: decodes the target loco
// address, CV number, and value to write, and dispatches a POM_WRITE
// command onto the command bus.
// ─────────────────────────────────────────────────────────────
void Z21Lan::_handleCvPomWrite(Z21Client& c, const uint8_t* data, uint8_t len) {
    if (len < 5) return;
    Command cmd{};
    cmd.type     = CmdType::POM_WRITE; cmd.sourceId = ModuleId::Z21_LAN;
    cmd.address  = _u16le(data);
    cmd.cv       = _u16le(data + 2) + 1;
    cmd.cvValue  = data[4];
    CommandBus::instance().dispatch(cmd);
...
// its address, step mode, speed+direction and function bits into a
// standard XpressNet 0xE4 loco-info frame, computes the XOR checksum,
// and wraps the result as a Z21 LAN_X reply via _buildXBusReply().
// ─────────────────────────────────────────────────────────────
void Z21Lan::_buildLocoInfo(uint8_t* buf, uint16_t& len, uint16_t addr) {
    ...
    xf.data[0]=0xE4; xf.data[1]=l.stepMode&0x07;
    xf.data[2]=ah;   xf.data[3]=al;
    xf.data[4]=spdDir;
...
// Z21_CLIENT_TIMEOUT milliseconds, deactivates the slot, clears its
...
void Z21Lan::_checkTimeouts() {
    ...
    for (uint8_t i = 0; i < Z21_MAX_CLIENTS; i++) {
        if (_clients[i].active &&
            now - _clients[i].lastSeenMs > Z21_CLIENT_TIMEOUT) {
