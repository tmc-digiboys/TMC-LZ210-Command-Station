...
void LenzUsb::loop() {
    ...
    if (_rxLen >= LENZ_USB_RX_BUF && Serial.available()) {
        Serial2.println("LenzUsb: buffer-overflow, herstel...");
        uint8_t err[3];
    ...
    if (_rxLen == 0) {
        ...
        if (_frameStartMs == 0) _frameStartMs = millis();
            ...
            Serial2.println("LenzUsb: onvolledig frame timeout");
            uint8_t err[3];
...
void LenzUsb::_processFrame(const uint8_t* buf, uint8_t len) {
    ...
    if (!sUsbInit) {
        ...
        sUsbXpressNet.setBroadcastCallback([](const uint8_t* data, uint8_t dlen) {
